...
import { deleteComics, fetchComics } from "./services";
...
function ComicGrid() {
  const { items, pagination, selectedItemIds, editingItemId } = useSelector((state) => ({
    items: state.comics.items,
    pagination: state.comics.pagination,
    selectedItemIds: state.comics.selectedItemIds,
    editingItemId: state.comics.editingItemId,
  }));
  const dispatch = useDispatch();

  const itemsByPage = getItemsByPage<ComicState>(items, pagination.page, pagination.perPage);
...
function GridControl() {
  ...
  const handleRemove = () => {
    ...
    withLoading(async () => {
      await deleteComics(selectedItemIds)
        .then(async () => {
...
function GridItemEditControl({ itemId }: { itemId: string }) {
  const { editingItemId, isFormDirty } = useSelector((state) => ({
    editingItemId: state.comics.editingItemId,
    isFormDirty: state.comics.isFormDirty,
  }));
  const dispatch = useDispatch();
