...
import { deleteWorks, fetchWorks } from "./services";
...
function WorkGrid() {
  const { items, pagination, selectedItemIds, editingItemId } = useSelector((state) => ({
    ...
    pagination: state.works.pagination,
    selectedItemIds: state.works.selectedItemIds,
    editingItemId: state.works.editingItemId,
  }));
  const dispatch = useDispatch();

  const itemsByPage = getItemsByPage<WorkState>(items, pagination.page, pagination.perPage);
    ...
    <>
      ...
      <Grid
        columns={[
          ...
          {
            label: "出版日",
            render: (item) => <span>{formatDateTimeFromUnixTimestamp(item.publishedDate, "YYYY/MM/DD")}</span>,
          },
          {
            label: "画像",
            css: css`
...
function GridControl() {
  ...
  const handleRemove = () => {
    ...
    withLoading(async () => {
      await deleteWorks(selectedItemIds)
        .then(async () => {
...
function GridItemEditControl({ itemId }: { itemId: string }) {
  const { editingItemId, isFormDirty } = useSelector((state) => ({
    editingItemId: state.works.editingItemId,
    isFormDirty: state.works.isFormDirty,
  }));
  const dispatch = useDispatch();
